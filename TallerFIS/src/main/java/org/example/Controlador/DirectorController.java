...
import org.example.VIsta.VerPedidoSingular;
...
public class DirectorController implements ActionListener {
    // Datos pa pruebas, no asustarse
    ...
    public DirectorController() {
        this.verPedidos = new VerPedidos(this);
        initDatos();
    }
    ...
    private Map<String, Object> obtenerPedido(Pedido pedido) {
        Map<String, Object> infoPedido = new HashMap<>();

        infoPedido.put("cliente", pedido.getCliente().getNombre());
        infoPedido.put("direccionEnvio", pedido.getDireccionEnvio());
        infoPedido.put("fechaPedido", pedido.getFechaPedido());
        return infoPedido;
    }

    private List<Map<String, Object>> obtenerLineasPedido(Pedido pedido){
        // Lista de líneas del pedido convertidas a mapas
        ...
        for (LineaPedido linea : pedido.getLineasPedido()) {
            ...
            lineaMap.put("codigo", linea.getArticulo().getCodigo());
            lineaMap.put("nombre", linea.getArticulo().getNombre());
            lineaMap.put("ordenada", linea.getCantidadOrdenada());
            lineaMap.put("pendiente", linea.getCantidadPendiente());
            lineas.add(lineaMap);
        }
        return lineas;
    }
    ...
    private void agregarPedidos(){
        ...
        for(Pedido pedido : pedidos){
            ...
            object[0] = pedido.getCodigo();
            object[1] = pedido.getCliente().getNombre();
            object[2] = pedido.getDireccionEnvio();
            object[3] = String.valueOf(pedido.getFechaPedido());
            pedidosObj.add(object);
        }

        for (Object[] pedidoObj : pedidosObj) {
            this.verPedidos.agregarFila(
                    (String) pedidoObj[0],
                    (String) pedidoObj[1],
                    (String) pedidoObj[2],
                    (String) pedidoObj[3]
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (comando.startsWith("verPedido:")) {
            int fila = Integer.parseInt(comando.split(":")[1]);
            System.out.println("Mostrando artículo de la fila " + fila);
            System.out.println(this.obtenerPedido(pedidos.get(fila)));
            new VerPedidoSingular(this.obtenerPedido(pedidos.get(fila)),this.obtenerLineasPedido(pedidos.get(fila)),this);
        }
    }
